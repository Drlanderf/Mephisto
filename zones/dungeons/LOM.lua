local MP = Mephisto
MP.zones["LOM"] = {}
local LOM = MP.zones["LOM"]

LOM.name = GetString(MP_LOM_NAME)
LOM.tag = "LOM"
...
LOM.bosses = {
	...
	[2] = {
		name = GetString(MP_LOM_SELENE),
	},
	[3] = {
		name = GetString(MP_LOM_AZUREBLIGHT_LURCHER),
	},
	[4] = {
		name = GetString(MP_LOM_AZUREBLIGHT_CANCROID),
	},
	[5] = {
		name = GetString(MP_LOM_MAARSELOK),
	},
	[6] = {
		name = GetString(MP_LOM_MAARSELOK_BOSS),
	},
}
...
function LOM.Init()
	...
	EVENT_MANAGER:RegisterForUpdate(MP.name .. LOM.tag .. "MovementLoop", 2000, LOM.OnMovement)
	EVENT_MANAGER:RegisterForEvent(MP.name .. LOM.tag, EVENT_PLAYER_COMBAT_STATE, LOM.OnCombatChange)
end

function LOM.Reset()
	EVENT_MANAGER:UnregisterForUpdate(MP.name .. LOM.tag .. "MovementLoop")
...
function LOM.OnMovement()
	...
	MP.OnBossChange(_, true, LOM.bosses[boss].name)
end
...
function LOM.OnBossChange(bossName)
	...
	MP.LoadSetup(LOM, pageId, index, true)
end
